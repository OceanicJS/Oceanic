...
export default class AutocompleteInteraction<T extends Types.Channels.AnyInteractionChannel | Types.Shared.Uncached = Types.Channels.AnyInteractionChannel | Types.Shared.Uncached> extends Interaction {
    ...
    user: User;
    constructor(data: Types.Interactions.RawAutocompleteInteraction, client: Client) {
        super(data, client);
    ...
    inCachedGuildChannel(): this is AutocompleteInteraction<Types.Channels.AnyTextableGuildChannel> {
        return this.channel instanceof GuildChannel;
    }

    /** Whether this interaction belongs to a private channel (PrivateChannel or uncached). The only difference on using this method over a simple if statement is to easily update all the interaction properties typing definitions based on the channel it belongs to. */
    inPrivateChannel(): this is AutocompleteInteraction<PrivateChannel | Types.Shared.Uncached> {
     ...
     */
    async result(choices: Array<Types.Interactions.AutocompleteChoice>): Promise<Types.Interactions.InteractionCallbackResponse<T>> {
        if (this.acknowledged) {
            throw new TypeError("Interactions cannot have more than one initial response.");
        }
        this.acknowledged = true;
        return this.client.rest.interactions.createInteractionResponse(this.id, this.token, { type: InteractionResponseTypes.APPLICATION_COMMAND_AUTOCOMPLETE_RESULT, data: { choices } }, true);
    }

    override toJSON(): Types.JSON.JSONAutocompleteInteraction {
        return {
            ...super.toJSON(),

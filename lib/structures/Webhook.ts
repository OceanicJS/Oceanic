...
export default class Webhook extends Base {
    private _cachedChannel?: Types.Channels.AnyTextableGuildChannel | null;
    ...
    user: User | null;
    constructor(data: Types.Webhooks.RawWebhook, client: Client) {
    ...
    get channel(): Types.Channels.AnyTextableGuildChannel | null | undefined {
        if (this.channelID !== null && this._cachedChannel !== null) {
            return this._cachedChannel ?? (this._cachedChannel = this.client.getChannel<Types.Channels.AnyTextableGuildChannel>(this.channelID));
        }
     ...
     */
    async deleteMessage(messageID: string, options?: Types.Webhooks.DeleteWebhookMessageOptions, token?: string): Promise<void> {
     ...
     */
    async edit(options: Types.Webhooks.EditWebhookOptions): Promise<Webhook> {
     ...
     */
    async editMessage<T extends Types.Channels.AnyTextableGuildChannel = Types.Channels.AnyTextableGuildChannel>(messageID: string, options: Types.Webhooks.EditWebhookMessageOptions, token?: string): Promise<Message<T>> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
     ...
     * @param token The token for the webhook, if not already present.
     */
    async editToken(options: Types.Webhooks.EditWebhookOptions, token?: string): Promise<Webhook> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
     ...
     * @param token The token for the webhook, if not already present.
     */
    async execute<T extends Types.Channels.AnyTextableGuildChannel>(options: Types.Webhooks.ExecuteWebhookWaitOptions, token?: string): Promise<Message<T>>;
    async execute(options: Types.Webhooks.ExecuteWebhookOptions, token?: string): Promise<void>;
    async execute<T extends Types.Channels.AnyTextableGuildChannel>(options: Types.Webhooks.ExecuteWebhookOptions, token?: string): Promise<Message<T> | void> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
        return this.client.rest.webhooks.execute<T>(this.id, t, options as Types.Webhooks.ExecuteWebhookWaitOptions);
    }
    ...
    async executeGithub<T extends Types.Channels.AnyTextableGuildChannel>(options: Record<string, unknown> & { wait?: true; }, token?: string): Promise<Message<T>>;
    async executeGithub<T extends Types.Channels.AnyTextableGuildChannel>(options: Record<string, unknown> & { wait?: boolean; }, token?: string): Promise<Message<T> | void> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
    ...
    async executeSlack<T extends Types.Channels.AnyTextableGuildChannel>(options: Record<string, unknown> & { wait?: true; }, token?: string): Promise<Message<T>>;
    async executeSlack<T extends Types.Channels.AnyTextableGuildChannel>(options: Record<string, unknown> & { wait?: boolean; }, token?: string): Promise<Message<T> | void> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
     ...
     * @param token The token for the webhook, if not already present.
     */
    async getMessage<T extends Types.Channels.AnyTextableGuildChannel>(messageID: string, threadID?: string, token?: string): Promise<Message<T>> {
        const t = this.token ?? token;
        if (!t) {
            throw new TypeError("Token is not present on webhook, and was not provided in options.");
        }
    ...
    override toJSON(): Types.JSON.JSONWebhook {
        return {
            ...super.toJSON(),
